...
public class RuleSetRecording extends EventRecording {
	...
	public boolean compare(EventRecording actual, PrintWriter out) {
		...
		if (children.size() != actualRuleSet.children.size()) {
			out.println("expected " + children.size() + " rules generating events, but got "
					+ actualRuleSet.children.size() + ": " + actualRuleSet.children);
			return false;
		}

		boolean match = true;
		Iterator<EventRecording> expectedIter = children.iterator(), actualIter = actualRuleSet.children.iterator();
